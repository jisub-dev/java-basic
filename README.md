...
# Section 13. 다형성3
* 역할과 구현으로 구분
  1. 단순(클라이언트는 구현 대상의 내부 구조를 몰라도 됨)
  2. 유연(클라이언트는 구현 대상의 내부 구조가 변경되어도 영향 X)
  3. 변경 용이(클라이언트는 구현 대상 자체를 변경해도 영향 X)
* 다형성의 본질   
**-> 클라이언트를 변경하지 않고, 서버의 구현 기능을 유연하게 변경할 수 있다**
* set 코드 자동 완성   
-> 인텔리제이에서 set 이라고 쓴 뒤 엔터를 누르면 아래와 같이 코드가 자동 완성된다
```java
...
* OCP(Open-Closed Princile)원칙   
: 새로운 기능의 추가나 변경사항이 생겼을 때, 기존 코드는 확장할 수 있어야 한다. 기존의 코드는 수정되지 않아야 한다.
